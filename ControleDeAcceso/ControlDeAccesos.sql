CREATE DATABASE  IF NOT EXISTS `ControlDeAccesos` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `ControlDeAccesos`;
...
CREATE TABLE `codigosAcceso` (
  ...
  `codigoAcceso` varchar(45) NOT NULL,
...
CREATE TABLE `empleados` (
  ...
  `fechaNacto` date,
  `documiento` varchar(15) NOT NULL, 
  `funcion` varchar(45) NOT NULL, 
  `efetivo` tinytext NOT NULL, 
  `fechaEntrada` date,
  `salario` float NOT NULL,
  `idDepartamento` int(11) NOT NULL,
  PRIMARY KEY (`idEmpleado`),
  KEY `fkDepart_Empleados_idx` (`idDepartamento`),
  CONSTRAINT `fkDepartEmpleados` FOREIGN KEY (`idDepartamento`) REFERENCES `departamentos` (`idDepartamento`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE `accesosEmpleados` (
  ...
  CONSTRAINT `fk_empleado1` FOREIGN KEY (`idEmpleado`) REFERENCES `empleados` (`idEmpleado`),
  CONSTRAINT `fk_codigoAcceso1` FOREIGN KEY (`idCodigoAcceso`) REFERENCES `codigosAcceso` (`idCodigoAcceso`)
